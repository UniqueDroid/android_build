...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^ud_") ; then
       UD_BUILD=$(echo -n $1 | sed -e 's/^ud_//g')
       NAM_VARIANT=$(echo -n $1 | sed -e 's/^ud_//g')
...
{
    target=$1
    UD_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/ud/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        MODVERSION=`sed -n -e'/ro\.ud\.version/s/.*=//p' $OUT/system/build.prop`
        ZIPFILE=update-ud-$MODVERSION-signed.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
